...
public class QRActivity extends AppCompatActivity implements View.OnClickListener, View.OnTouchListener {
    ...
    int MediaFlag=0;
    String[] columns = {"artical_id", "gallery_id", "artical_name", "artical_description","media_id"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(LoginActivity.ENGLISH_FLAG) {
            Cursor cursor = writeDatabase.query("e_artical_table", columns, "gallery_id= ?", new String[]{str}, null, null, null);//Searching image_id in artical_table
        ...
        if(LoginActivity.HINDI_FLAG) {
            Cursor cursor = writeDatabase.query("h_artical_table", columns, "gallery_id= ?", new String[]{str}, null, null, null);//Searching image_id in artical_table
            if (cursor.getCount() == 0) {
                Toast.makeText(getApplicationContext(), "No record found", Toast.LENGTH_SHORT).show();
            } else {
                cursor.moveToNext();
        ...
        if(LoginActivity.JAPANESE_FLAG) {
            Cursor cursor = writeDatabase.query("j_artical_table", columns, "gallery_id= ?", new String[]{str}, null, null, null);//Searching image_id in artical_table
            if (cursor.getCount() == 0) {
                Toast.makeText(getApplicationContext(), "No record found", Toast.LENGTH_SHORT).show();
            } else {
                cursor.moveToNext();
        ...
        str="/Museum_Images/"+str;
        String path = Environment.getExternalStorageDirectory().toString()+str;//Adding the path of image
